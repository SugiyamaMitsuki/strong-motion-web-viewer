...
export async function downloadAnalysisZip(
  ...
  zip.file('fourier_acceleration.csv', buildFourierCsv(waveforms, 'acceleration'));
  zip.file('fourier_velocity.csv', buildFourierCsv(waveforms, 'velocity'));
  zip.file('fourier_displacement.csv', buildFourierCsv(waveforms, 'displacement'));
